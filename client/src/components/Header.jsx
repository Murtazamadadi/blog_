...
export default function Header() {
    ...
    const handleSignOut=async()=>{
      ...
      try{
        const res=await fetch(`${import.meta.env.VITE_REACT_APP_BACKEND_URL}/api/user/sign-out`,{
          method:"POST"
        })
  ...
  return (
    <Navbar className='border-b-2'>
      ...
      <div className='flex gap-2 md:order-2'>
        ...
        <Navbar.Toggle aria-controls="navbar-collapse" aria-expanded={false}/>
      </div>
      <Navbar.Collapse>
        <Navbar.Link active={path === '/'} as={'div'}>
          <Link to='/' className=' ml-7'>خانه</Link>
        </Navbar.Link>
        <Navbar.Link active={path === '/about'} as={'div'}>
          <Link to='/about'>درباره ما</Link>
        </Navbar.Link>
        <Navbar.Link active={path === '/projects'} as={'div'}>
          <Link to='/projects'>پروژها</Link>
        </Navbar.Link>
      </Navbar.Collapse>
    </Navbar>
  );
